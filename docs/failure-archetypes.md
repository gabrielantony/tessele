...
## Asserção de geometria arredondada não vê fenda de sub-pixel
- **Archetype:** dividir uma largura entre N irmãos (`flex-1`, `width: 20%`, grid de frações) quase nunca cai em pixel inteiro, e nas junções cuja fração arredonda para fora dos dois lados sobra uma coluna de 1px que **nenhum** dos dois pinta. O que aparece nela é o que está atrás — no nosso caso o `bg-canvas` creme atrás da cortina, em força total, e não uma linha esmaecida. Duas propriedades tornam isso quase imune a review: **depende da largura** (a 1280px cinco painéis dão 256px exatos e nada aparece; a 1512px dão 302,4px e duas das quatro junções vazam), e **o teste certo passa**. A asserção estava no nível certo (geometria, não presença de token) e ainda assim era cega: ela comparava `boxes[i].left - boxes[i-1].right` **arredondado**, e 604,8 menos 604,8 dá zero enquanto os pixels renderizados têm buraco. Rigor no lugar certo, resolução errada.
- **Ask when planning:** para cada layout que reparte largura ou altura entre irmãos: as bordas caem em pixel inteiro em TODA largura suportada, ou existe fração? Se existe, os irmãos se sobrepõem (margem negativa de 1px) em vez de apenas se tocarem — sobreposição entre caixas da mesma cor é invisível e não tem largura em que falhe. E a asserção: se o defeito é uma coluna renderizada, medir **pixels** (screenshot desenhado num canvas e varrido), não `getBoundingClientRect` arredondado; se medir geometria, medir sem arredondar e proibir apenas fenda positiva. Testar sempre numa largura divisível e numa não divisível — a divisível é controle, e sozinha ela aprova qualquer construção.
- **Escaped in:** costura de 1px entre os painéis da cortina, vazando `rgb(250,249,245)` em x=604 e x=1209 a 1512px, com "the shut curtain tiles the viewport with no holes" verde nas duas vezes que o defeito foi reportado (2026-09-02)

## z-index sem contexto de empilhamento alcança fora do dono
- **Archetype:** um `z-index` levantado para cobrir algo *dentro* de um componente compete, na verdade, com todos os irmãos dele, porque sem `position` + `z-index` no elemento raiz do componente não existe contexto que o confine. O overlay passa a cobrir a seção seguinte — e a seção seguinte quase sempre pinta um pouco fora da própria caixa (`sticky` com `-translate-y-*`, sombra, ring, margem negativa). Quando as duas superfícies são da **mesma cor**, o sintoma não parece sobreposição: parece que o conteúdo não chegou, e depois chega atrasado quando a faixa sai da tela. Também não dá para pegar por hit-test, porque um overlay decorativo é `pointer-events-none` e o `elementFromPoint` passa direto por ele — a checagem tem de ser sobre ordem de pintura, não sobre clique.
- **Ask when planning:** todo `z-index` acima de 0: qual é o menor elemento que ele precisa cobrir, e o ancestral que representa esse escopo estabelece contexto de empilhamento (`position` não-static **e** `z-index` não-auto)? E na direção oposta, para cada seção: o que ela pinta fora da própria border box, e quem é o vizinho cuja caixa ocupa esse espaço? Empatar em `z-index: 0` é a resposta entre irmãos — a ordem de documento decide, e quem vem depois ganha por vir depois.
- **Escaped in:** o overlay `z-10` da cortina cobrindo os 96px de frase que a QuoteSection pinta acima do próprio topo, nas duas cores accent (2026-09-02)
